...
- **Today** for recording daily and monthly transactions.
- **History** for browsing, searching, and filtering past transactions.
- **Analysis** for viewing totals, trends, and comparisons.
- **Settings** for managing templates, categories, themes, recurring schedules, currencies, backups, and iCloud Sync.
...
- Choose **Keep This Device Data** to preserve the records on this device and upload or merge them with iCloud.
- Choose **Restore Existing iCloud Data** only when iCloud already contains the Money Diary records you want. This replaces the records currently on the device after first making a local safety backup.
...
- **Export JSON** creates a full Money Diary backup file.
- **Export CSV** creates a spreadsheet-compatible file containing transactions.
- **Import** reads a supported JSON or CSV file.
- **Restore from Auto-Backup** lets you recover from a recent local backup.

Store exported backup files somewhere secure. They may contain personal financial information.

When importing a JSON backup:

- **Merge** keeps current records and adds records that are not already present. This is the recommended choice in most cases.
- **Replace All** deletes current app data and restores the backup. Review the confirmation carefully because the replacement may also reach other devices when iCloud Sync is enabled.
